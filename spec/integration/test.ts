...
describer('notification api with a live service', function () {
  ...
  beforeEach(() => {
    ...
    whitelistNotifyClient = new NotifyClient({ baseUrl, apiKeyId: whitelistApiKeyId });
    receivedTextClient = new NotifyClient({ baseUrl, apiKeyId: inboundSmsKeyId });
  ...
  describe('notifications', () => {
    ...
    it('send email notification', () => {
      ...
      const options = {personalisation: personalisation, reference: clientRef};
    ...
    it('send email notification with email_reply_to_id', () => {
      const postEmailNotificationResponseJson = require('./schemas/v2/POST_notification_email_response.json');
      const options = {personalisation: personalisation, reference: clientRef, emailReplyToId: emailReplyToId};
